...
public class GameManager : MonoBehaviour {
    ...
    public void die(string reason) {
        if (Util.wm.godmode || (distance > Util.adLifeMinDist && (Util.wm.adWatchTimeLife <= 0 && Util.wm.gamesSinceAdWatch > Util.adLifeMinGames) && Random.Range(0, 100f) < Util.adLifeProbability && firstTry)) {
    ...
    void updateZone() {
        ...
        if (zoneID == 3) {
            Invoke("spawnAsteroidBG", Random.Range(0.35f, 0.8f));
            if (Random.Range(-1f, 1f) < 0) Invoke("spawnAsteroidBG", Random.Range(0.5f, 1.6f));
        }
    }
